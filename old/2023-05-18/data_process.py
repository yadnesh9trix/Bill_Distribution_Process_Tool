...
last1 = today - timedelta(days=0)
yest_fmt  = last1.strftime("%d%m%Y")
...
def find_arrears_cntbill(outpth, inppath, ytddata_df, property_bill_df, property_list_df):
    ...
    # merge_plist_arrearsTY1 = merge_plist_arrearsTY.merge(current_TY, on='propertykey', how='left')
    #
    # merge_plist_arrearsTY1.to_excel(outpth + "test.xlsx", index=False)
    ###-----------------------------------------------------------------------------------------------------------------
    ...
    # ==========================================================================================================================================================================
def property_FY_process(outpth,inppath,ytddata_df, property_receipt_df,property_list_df,unique_pkey_df):
    ...
    return plist_arrange, merge_maxfyrmth_uniqpid_df_Year_r,property_finmth_df,filterdata_lessthan_2022



def newlist_receiptdate(inppath,outpth,plist_arrange, merge_maxfyrmth_uniqpid_df_Year_r,
    ...
    all_Bill_arrers = AllBillArrears[['propertykey', 'Arrears', 'Arrears_Flag',
                                      'Current_Bill', 'CurrentBill_Flag','assessmentdate']]
    ...
    merge_maxreciptdate_finaldump['receiptdate'] = merge_maxreciptdate_finaldump['receiptdate'].fillna(merge_maxreciptdate_finaldump['assessmentdate'])

    merge_maxreciptdate_finaldump = merge_maxreciptdate_finaldump.drop(columns='assessmentdate',axis=1)
...
###==================================================================================================================================================
    ...
    merge_maxreciptdate_finaldump['receiptdate'] = merge_maxreciptdate_finaldump['receiptdate'].astype(str)
    ## select only pid column from ytd data
    ...
    plist_rearrange_data1 = merge_maxreciptdate_finaldump.merge(ytddata1, on='propertycode', how='left')

    plist_rearrange_data1['Last Payment Date'] = plist_rearrange_data1['receiptdate_typaid'].fillna(plist_rearrange_data1['receiptdate'])
    plist_rearrange_data1['Last Paidamount'] = plist_rearrange_data1['paidamount_TY'].fillna(plist_rearrange_data1['paidamount'])

    # plist_rearrange_data1['new update date'] = pd.to_datetime(plist_rearrange_data1['new update date'],
    #                                                           errors='coerce').dt.date
    #------------------------------------------------------------------------------------------------------------------------------------------
    ## Read YTD data current Year
    # plist_rearrange_data1_new = merge_maxreciptdate_finaldump.merge(plist_rearrange_data1, on='propertycode', how='left')

    # plist_rearrange_data1_new['Last Payment Date'] = plist_rearrange_data1_new['receiptdate_typaid1'].fillna(plist_rearrange_data1_new['receiptdate'])
    # plist_rearrange_data1_new['Last Paidamount'] = plist_rearrange_data1_new['paidamount_TY1'].fillna(plist_rearrange_data1_new['paidamount'])

    #------------------------------------------------------------------------------------------------------------------------------------------
    illegal_fine_plist = pd.read_csv(inppath + "Property_Shasti_list.csv")
    ...
    # illegal_fine_plist["propertycode"] = illegal_fine_plist["propertycode"].replace("1150411115..00", "1150411115.00")
    # illegal_fine_plist['propertycode'] = [float(str(i).replace("`", "")) for i in illegal_fine_plist['propertycode']]
    ...
    #     ['propertycode', 'illegaltotalbal', 'otherstotalbal', 'Shasti_Flag']]
    ...
    ppp = property_list_df[
        ['propertykey', 'zone', 'gat', 'propertyname', 'propertyaddress', 'usetypekey', 'finalusetype',
         'subusetypekey', 'constructiontypekey', 'occupancykey', 'own_mobile','assessmentdate']]
    ppp['propertykey'] = ppp['propertykey'].drop_duplicates()

    # shasti_merge_maindf = ppp.merge(illegal_fine_plist, on='propertykey', how='inner')
    # plist_rearrange_data = shasti_merge_maindf.merge(plist_rearrange_data1_new, on='propertykey', how='inner')
    shasti_merge_maindf = plist_rearrange_data1.merge(illegal_fine_plist, on='propertykey', how='left')
    plist_rearrange_data = ppp.merge(shasti_merge_maindf, on='propertykey', how='inner')
    ...
    # plist_data = plist_rearrange_data[['propertykey', 'Zone_Type',
    #                                                'gat_name', 'propertycode', 'receiptdate', 'paidamount',
    #                                                'receiptdate_typaid', 'paidamount_TY','new update date','new update paidamount', 'Arrears', 'Arrears_Flag',
    #                                                'Current_Bill','CurrentBill_Flag',
    #                                                'Arrears_from2019', 'Arrears_from2017', 'paid_in_March_LY',
    #                                                'paid_in_LY','paid_in_TY', 'not_paid_LY', 'paid_by_dec_LY', 'Use_Type',
    #                                                'Construction_Type', 'Occupancy_Type', 'Subuse_Type', 'propertyname',
    #                                                'own_mobile', 'propertyaddress']]
    plist_rearrange_data['Last Payment Date'] = pd.to_datetime(plist_rearrange_data['Last Payment Date'],
                                                               infer_datetime_format=False, format='%Y-%m-%d',
                                                               errors='ignore')
    # plist_rearrange_data['assessmentdate'] = pd.to_datetime(plist_rearrange_data['assessmentdate'])
    plist_data = plist_rearrange_data[['propertykey', 'Zone_Type',
                                                   'gat_name', 'propertycode','assessmentdate','Last Payment Date','Last Paidamount', 'Arrears', 'Arrears_Flag',
                                                   'Current_Bill','CurrentBill_Flag','paid_in_March_LY', 'Shasti_Flag',
                                                   'paid_in_LY','paid_in_TY', 'not_paid_LY',  'Use_Type',
                                                   'Construction_Type', 'Occupancy_Type', 'Subuse_Type', 'propertyname',
                                                   'own_mobile', 'propertyaddress']]
    # ====================================================================================================================================================================
    ...
    plist_data.to_excel(outpth + "/" + f"Master_Plist_Data.xlsx", index=False,
                                   sheet_name="MasterArrearsPlist",encoding='utf8')
    # #---------------------------------------------------------------------------------------------------------------------------------------
